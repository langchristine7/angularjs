app.factory('ingoFactory', function(){
  ...
  var factory = {
    ...
    geoReps: geoReps,
    aocsTransformed: function(identChar){
      var t = [];
...
app.factory('ingoDataFactory', ['$http', function($http){
  var factory = {
    ingos: false,
    getIngos: function(){
      return $http.get('js/ngos.json').then(
        function(res){
          // pour _.map voir underscore.js qui permet de faire plein de chose
          return factory.ingos = _.map(res.data, function(ingo, i){
            i = i | 0;
            i++;
            return{
              id: i,
    ...
    getIngo: function(id){
       return ngo = _.findWhere(factory.ingos, {id:id});
    }
  };
...
app.factory('ingoInterface', function(){
  var factory = {
    menu:{
      signIn:{
        text:'Sign In',
